...
void PrescribedController::extendConnectToModel(Model& model)
{
    ...
    if(!getProperty_controls_file().empty()){
        ...
        for(int i=0; i<ncols; ++i){
 		...
 		const Component* found1=getModel().findComponent(actName);
                if(found >= 0 || found1){ // found a corresponding actuator
                    controls.getDataColumn(controls.getStateIndex(actName), 
                                            data);
                    Function* pfunc=createFunctionFromData(actName, time, data);
		      if (found1) {
                        const string& actN=actName.substr(1,actName.length()-1);
			prescribeControlForActuator(actN, pfunc);
			continue;}
    ...
    prescribeControlForActuator(int index, Function *prescribedFunction)
{
   ...
   // OPENSIM_THROW_IF(index >= getActuatorSet().getSize() && index >= get_ControlFunctions().getSize(),
   ...
   //         (size_t)getActuatorSet().getSize() - 1);
    ...
    upd_ControlFunctions().set(index, prescribedFunction);  
        auto& fun=upd_ControlFunctions()[index];
        GCVSpline* fn = dynamic_cast<GCVSpline*>(&fun);
        //cout<<fun.getArgumentSize()<<fn->getNumberOfPoints()<<endl;
        //for(int i=0;i<fn->getSize();i++)cout<<fn->getX(i)<<":"<<fn->getY(i)<<endl;
        //        

    //cout<<"set function to:"<<index<<endl;
}
...
//    cout<<getProperty_actuator_list().getValue(i)<<endl;
    if(index < 0 )
                        ...
                        const Array<double>& time, const Array<double>& data)
{
    ...
    //for (int i=0;i<time.getSize();i++) cout<<time[i]<<":"<<data[i]<<endl;
